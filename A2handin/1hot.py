...
model.add(PositionalEncoding(vec_dim=vocab_size, seq_len=seq_len))

# Add the transformer layer to the model
model.add(TransformerLayer(vec_dim=vocab_size, key_dim=32, num_heads=6, dff=256))
model.add(TransformerLayer(vec_dim=vocab_size, key_dim=32, num_heads=6, dff=256))
model.add(TransformerLayer(vec_dim=vocab_size, key_dim=32, num_heads=6, dff=256))
model.add(TransformerLayer(vec_dim=vocab_size, key_dim=32, num_heads=6, dff=256))
model.add(TransformerLayer(vec_dim=vocab_size, key_dim=32, num_heads=6, dff=256))
...
learning_rate = CustomSchedule(vocab_size)
...
if LoadFromFile:
    # Load the model
    model.load_weights("1HotTransformer.h5")
else:
    # Train the model
    print("Training model...")
    model.fit(train_data, epochs=epochs)
    model.save_weights("1HotTransformer.h5")
